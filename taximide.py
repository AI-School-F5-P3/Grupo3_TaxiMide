...
class Taximetro:
    ...
    def crear_tabla_registros(self):
        try: #se establece un bloque try para manejar a través de excepciones posibles errores que puedan surgir.
            self.conexion_bd = sqlite3.connect("taximetro.db") # Se establece una conexión con .connect a la librería sqlite3, el enrutamiento se establece a la base de datos taximetro.db declarado en el atributo en la lin 93
            cursor = self.conexion_bd.cursor() # Se declará el cursor, realiza un seguimiento de la posición en el conjunto de resultados y permite realizar varias operaciones fila por fila en un conjunto de resultados, con o sin retorno a la tabla original. Es decir, los cursores devuelven conceptualmente un conjunto de resultados basado en las tablas de las bases de datos.
            cursor.execute(''' 
