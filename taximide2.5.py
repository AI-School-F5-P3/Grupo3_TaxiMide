import json
import hashlib #importamos librerias
import time
import logging
import argparse
...
class Taximetro:
    ...
    def cargar_configuracion(self):
        try:
            with open('config.json', 'r') as f:
                ...
                self.ultima_contraseña = config.get('ultima_contraseña')
                self.autenticado = False  # Forzar autenticación al inicio
                logging.info("Configuración cargada correctamente.")
        except FileNotFoundError:
            logging.warning("Archivo de configuración no encontrado. Utilizando valores por defecto.")
        except json.JSONDecodeError:
            logging.error("Error al cargar la configuración desde el archivo JSON.")
            messagebox.showerror("Error", "Error al cargar la configuración. Utilizando valores por defecto.")

    def guardar_configuracion(self):
        config = {
            'password_hash': self.password_hash,
            'ultima_contraseña': self.ultima_contraseña
        }
        try:
            ...
            logging.error("Error al guardar la configuración en el archivo JSON.")
    ...
    def verificar_password(self, entered_password):
        return self.ultima_contraseña and self.ultima_contraseña == self.hash_password(entered_password)
    ...
    def cambiar_contraseña(self):
        ...
        while True:
            ...
            if new_password is None:
                return  # Si se cancela la entrada de contraseña, salir del método
            ...
            if new_password == confirm_password:
                ...
                retry = messagebox.askyesno("Error", "¿Quieres intentarlo de nuevo?")
                if not retry:
                    break
