# incrementamos tamaño de los contadores y añadimos logo en cabecera de ventana
import time
import logging
import argparse
...
class Taximetro:
    ...
    def iniciar_carrera(self, root):
        ...
        try:
            image = Image.open("/Users/cash/Desktop/F5/Grupo3_TaxiMide/taximide_vertical_logo.png")  # Ruta absoluta al archivo de imagen
            image = image.resize((250, 100), Image.Resampling.LANCZOS)  # Resize the image if needed
    ...
    def finalizar_carrera(self):
        ...
        # Verificar si estado_label está inicializado antes de intentar modificarlo
        if hasattr(self, 'estado_label'):
            self.estado_label.config(text="Taxi en parado.", fg="red")
        else:
            logging.warning("estado_label no está inicializado. No se puede actualizar el estado visual.")

        # Verificar si root está inicializado antes de intentar cerrar la aplicación
        if hasattr(self, 'root') and hasattr(self.root, 'quit'):
            self.root.quit()
        else:
            logging.warning("root no está inicializado correctamente. No se puede cerrar la aplicación.")
